...
{
  static char storage[] = "Storage Project Per Disk Semaphore Final Version";
  char *buf;
  int buf_size;
  int dest_desc;
  int source_desc;
  ...
  pid_t pid;
  unsigned int smajor = 0, sminor = 0, slow = 0, dmajor = 0, dminor = 0, dlow = 0, skey = 0, dkey = 0;
  ...
  buf_size = ST_BLKSIZE (dsb);

#if HAVE_STRUCT_STAT_ST_BLOCKS
  if (x->sparse_mode == SPARSE_AUTO && S_ISREG (dsb.st_mode))
    {
      /* Use a heuristic to determine whether SRC_PATH contains any
	 sparse blocks. */

      if (fstat (source_desc, &ssb))
	{
	  error (0, errno, _("cannot fstat %s"), quote (src_path));
	  return_val = -1;
	  goto close_src_and_dst_desc;
	 ...
	 at least one of the blocks in the file is a hole. */
	  ...
	  && ssb.st_size / ST_NBLOCKSIZE > ST_NBLOCKS (ssb))
	make_holes = 1;
      
      if(ssb.st_size > SIZE_THRESHOLD) {
	  dminor = dsb.st_dev & 0xFF;
	  dmajor = dsb.st_dev >> 8;

	  if( (dmajor == 8) ||
	      ((dmajor >= 65) && (dmajor <= 71)) ||
	      ((dmajor >= 128) && (dmajor <= 131)) )
	    dlow = dminor >> 4;
	  ...
	  dkey = (((KEY << 8) | dmajor) << 8) | dlow;

	  //fprintf(stdout, "dkey = %X\ndmajor = %X\ndminor = %X\ndlow = %X\n", dkey, dmajor, dminor, dlow);

	  sminor = ssb.st_dev & 0xFF;
	  smajor = ssb.st_dev >> 8;

	  if( (smajor == 8) ||
	      ((smajor >= 65) && (smajor <= 71)) ||
	      ((smajor >= 128) && (smajor <= 131)) )
	    slow = sminor >> 4;
	  ...
	  skey = (((KEY << 8) | smajor) << 8) | slow;

	  //fprintf(stdout, "skey = %X\nsmajor = %X\nsminor = %X\nslow = %X\n", skey, smajor, sminor, slow);

	  if(skey == dkey) {
	    if((semid = semget(skey, 1, IPC_CREAT|IPC_EXCL|0666)) == -1) {
	      if(errno == EEXIST) {
		if((semid = semget(skey, 1, 0666)) == -1) {
		  fprintf(stderr, "pid = %d (%s) %d# semget error, errno = %d\n", pid, __FILE__, __LINE__, errno);
		  exit(-1);
		}
	      }
	      else {
	    ...
	    if((semid1 = semget(skey, 1, IPC_CREAT|IPC_EXCL|0666)) == -1) {
	      if(errno == EEXIST) {
		if((semid1 = semget(skey, 1, 0666)) == -1) {
		  fprintf(stderr, "pid = %d (%s) %d# semget error, errno = %d\n", pid, __FILE__, __LINE__, errno);
		  exit(-1);
		}
	      }
	    ...
	    lock_sem(semid1);
	    if((semid2 = semget(dkey, 1, IPC_CREAT|IPC_EXCL|0666)) == -1) {
	      if(errno == EEXIST) {
		if((semid2 = semget(dkey, 1, 0666)) == -1) {
		  fprintf(stderr, "pid = %d (%s) %d# semget error, errno = %d\n", pid, __FILE__, __LINE__, errno);
		  exit(-1);
		}
	      }
  ...
  if (x->sparse_mode == SPARSE_AUTO && S_ISREG (ssb.st_mode) && ssb.st_size > SIZE_THRESHOLD)
    {
      if(skey == dkey)
	unlock_sem(semid);
      else {
